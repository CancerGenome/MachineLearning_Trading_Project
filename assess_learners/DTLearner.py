...
class DTLearner(object):
    ...
    def build_tree(self, data):
        ...
        if(sample_n <= self.leaf_size) or (np.all(y==y[0])) : #means all Y are same or leaf size smaller than others
            ...
            #correlation2 = np.abs(np.corrcoef(x[:,1], y, rowvar= False))[0,1]
            #print(f"Correlation2:{correlation2}")
            max_index = np.argmax(correlation)
            #print(f"Max_index:{max_index}")
            split_value = np.median(x[:, max_index])
            if(np.all(data[:,max_index] <= split_value)):
                return np.array([["Leaf", np.mean(y), "NA","NA"]], dtype = object) # the branch end
            left_tree = self.build_tree(data[data[:,max_index] <= split_value])
            right_tree = self.build_tree(data[data[:,max_index] > split_value])
            root = np.array([[max_index,split_value,1, left_tree.shape[0] + 1]], dtype = object)
