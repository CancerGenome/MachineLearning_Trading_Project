...
def testPolicy(symbol = "NVTA", sd=dt.datetime(2008, 1, 1), ed=dt.datetime(2009,12,31), sv = 100000, return_sv = 0, commission = 9.95, impact = 0.005):
    ### For Test 
    #symbol = 'NVTA'
    ...
    price = get_data(symbol_list, pd.date_range(sd,ed) , addSPY = False) 
    price = price.dropna()
    ...
    for i in range(15,n+1):
        ...
        ## Add commision and impact, buy impact should be positive, sell impact should be negative
        if Hold['Buy'][i] > 0:
...
def benchmark(symbol = "NVTA", sd=dt.datetime(2008, 1, 1), ed=dt.datetime(2009,12,31), sv = 100000):
    price_bench = get_data([symbol], pd.date_range(sd, ed), addSPY=False ) 
    price_bench = price_bench.dropna()
...
def test_code():
    ### Prepare input
    symbol = 'NVTA'
    sd=dt.datetime(2015, 2, 1)
    ed=dt.datetime(2019,11,1)
    sv = 100000
    symbol_list = list(symbol.split(" "))
    price = get_data(symbol_list, pd.date_range(sd,ed) , addSPY=False) 
    price = price.dropna()
    ...
    personal = testPolicy(symbol = symbol, sd = sd, ed = ed, sv = sv, return_sv = 1, commission = 0 , impact = 0 )
    personal_buy = testPolicy(symbol = symbol, sd = sd, ed = ed, sv = sv, return_sv = 0, commission =0, impact = 0 )
