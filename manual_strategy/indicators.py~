...
def indicator_plot():
    ...
    price_jpm = get_data(['JPM'], pd.date_range('2008-01-01','2009-12-31') ) # get price of SPY of all days
    price_jpm = price_jpm[['JPM']]
    price_jpm = price_jpm/price_jpm.iloc[0]
    sma = SMA(price_jpm, window = 15)
    bb = BB(price_jpm, window = 15)
    pp = PP(price_jpm, window = 15)
...
#---- Plot SMA
    ...
    line1,=plt.plot(price_jpm,label = 'Price', color = color[1])
    line2,=plt.plot(sma, linestyle = "--", label = "SMA", color = color[8])
    line3,=plt.plot(sma/price_jpm, linestyle = "--", label = 'SMA/Price', color = color[7])
...
#---- Plot Pivot Point
    ...
    line3,=plt.plot(pp/price_jpm, linestyle = "--", label = 'Pivot Point/Price', color = color[7])
    plt.legend(handles=[line1,line2,line3], loc=4)    
    plt.savefig('PP.png')
    plt.close(fig)
